#상속
#1. 부모클래스가 생성자가 없을 때
#1)부모클래스(super 클래스)
'''class Animal:
    def speak(self):
        print("동물이 소리를 냅니다.")
    
    def move(self):
        print("동물이 움직입니다.")

#2)자식글래스(sub 클래스)
class Cat(Animal):
    def meow(self):
        print("야옹!")

cat=Cat()
cat.speak() #부모 클래스를 호출하지 않아도 사용가능
cat.move() #부모 클래스를 호출하지 않아도 사용가능
cat.meow()'''


#2. 부모클래스에 생성자가 있을 때
#1) 부모클래스(super 클래스)
'''class Animal():
    def __init__(self,name) -> None:
        self.name=name
    def speak(self):
        print(f"{self.name}가 소리를 냅니다.")

    def move(self):
        print(f"{self.name}가 움직입니다.")

#2) 자식클래스(sub 클래스)
class Cat(Animal):
    def __init__(self, name,sound="야옹") -> None: #순서는 상관없지만 관례상 부모 변수가 먼저 나오고 자식 변수가 나옴
        super().__init__(name)#self.__name을 부모클래스에서 호출
        self.sound=sound
    
    def meow(self):
        print(f"{self.name}가 {self.sound} 짖습니다.")

cat=Cat("장화","캬악")
cat.speak()
cat.move()
cat.meow()'''

#다중상속
'''class Engine:
    def __init__(self,horsepower) -> None:
        self.horsepower=horsepower

class Wheels:
    def __init__(self,count) -> None:
        self.count=count

class Car(Engine,Wheels):
    def __init__(self, horsepower, count) -> None:
        Engine.__init__(self,horsepower)
        Wheels.__init__(self,count)

    def info(self):
        print(f"이 자동차는 {self.horsepower}마력과 {self.count}개의 바퀴를 가지고 있다.")

car=Car(100,4)
car.info()
print(Car.mro())'''

'''class Engine:
    def __init__(self,horsepower,count) -> None:
        self.horsepower=horsepower
        self.count=count

class Wheels:
    def __init__(self,count) -> None:
        self.count=count

class Car(Engine,Wheels):
    def __init__(self,horsepower,count,wheel_count) -> None:
        Engine.__init__(self,horsepower)
        Wheels.__init__(self,wheel_count)

    def info(self):
        print(f"이 자동차는 {self.horsepower}마력과 {self.count}개의 바퀴를 가지고 있다.")

car=Car(100,4)
car.info()
print(Car.mro())'''

#오버라이딩
'''class Parent:
    def greet(self):
        print("안녕하세요,부모클래스")

class Child(Parent):
    def greet(self):
        print("안녕하세요, 자식클래스")

p=Parent()
c=Child()
p.greet()
c.greet()        
'''

'''class Parent:
    def greet(self):
        print("안녕하세요,부모클래스")

class Child(Parent):
    def greet(self):
        super().greet() #부모클래스에 없는 greet() 호출
        #print("안녕하세요, 자식클래스")

p=Parent()
c=Child()
p.greet()
c.greet()'''

#실습. 상속과 오버라이딩 => 주어진 조건에 따라 Food 클래스에서 current_date를 변수로 두어야함
'''class Product:
    def __init__(self, name, price, quantity):
        self.name = name
        self.price = price
        self.quantity = quantity

    # 재고 업데이트 메서드
    def update_quantity(self, amount):
        self.quantity += amount
        print(f"{self.name} 재고가 {amount}만큼 {'증가' if amount > 0 else '감소'}했습니다. 현재 재고: {self.quantity}")

    # 상품 정보 출력 메서드
    def display_info(self):
        print(f"상품명: {self.name}")
        print(f"가격: {self.price}원")
        print(f"재고: {self.quantity}개")

class Electronic(Product):
    
    def __init__(self, name, price, quantity,warranty):
        super().__init__(name, price, quantity)
        self.warranty=warranty

    def extend_warranty(self,months):
        self.warranty+=months
        print(f"보증기간이 {months}개월 연장되었습니다. 현재 보증기간:{self.warranty}개월") 

    def display_info(self):
        super().display_info()
        print(f"보증기간:{self.warranty}개월")

class Food(Product):
    def __init__(self, name, price, quantity,expired_date):
        super().__init__(name, price, quantity)
        self.expired_date=expired_date

    def is_expired(self,current_date):
        if current_date<self.expired_date:
            print(f"{self.name}은(는) 유통기한이 지났습니다.")
        elif current_date==self.expired_date:
            print(f"{self.name}의 유통기한은 오늘입니다.")
        else:
            print(f"{self.name}은(는) 유통기한이 지나지 않았습니다.")

    def display_info(self):
        super().display_info()

x=Electronic("스마트TV",1500000,5,24)
x.display_info()
x.extend_warranty(12)
x.display_info()
print()
y=Food("사과",3000,50,2024-12-28)
y.is_expired(2024-11-28)
y=Food("사과",3000,50,2024-10-28)
y.is_expired(2024-11-28)
y.display_info()'''

#실습. 상속과 오버라이딩 강사님
'''class Electronic(Product):
    def __init__(self, name, price, quantity, warranty_period=12):
        super().__init__(name, price, quantity)
        self.warranty_period=warranty_period

    #보증기간 연장
    def extend_warranty(self,months):
        self.warranty_period+=months
        print(f"보증기간이 {months}개월 연장, 현재 보증기간: {self.warranty_period}개월")
    
    #오버라이딩
    def display_info(self):
        super().display_info()
        print(f"보증기간: {self.warranty_period}개월")

class Food(Product):
    def __init__(self, name, price, quantity, expiration_date):
        super().__init__(name, price, quantity)      
        self.expiration_date=expiration_date
    
    #유통기한 확인
    def is_expired(self, current_date):
        if current_date>self.expiration_date:
            print(f"{self.name}은/는 유통기한이 지났습니다.")
        else:
            print(f"{self.name}은/는 유통기한이 지나지 않았습니다.")
        
    def display_info(self):
        super().display_info()
        print(f"유통기한:{self.expiration_date}")


tv=Electronic("삼성스마트TV", 1500000, 2, 24)
tv.display_info()
tv.extend_warranty(12)
tv.display_info()

milk=Food("서울우유",3000,30,"2024-12-10")
milk.display_info()
milk.is_expired("2024-11-28")
milk.is_expired("2024-12-28")
'''

#추상화
#1.추상클래스
'''from abc import ABC, abstractmethod

class PaymentSystem(ABC): # ABC를 꼭 상속받아야함

    #추상메서드
    @abstractmethod
    def authenticate(self):
        pass
        #추상매서드 내 코드 작성 금지

    @abstractmethod
    def process_payment(self,amount):
        pass

    def payment_info(self, amount):
        print(f"{amount}원 결재가 완료되었습니다.")


class kakaoPay(PaymentSystem):
    def authenticate(self):
        print("카카오페이 인증완료되었습니다.")


    def process_payment(self, amount):
        print(f"카카오페이로 {amount}원을 결재합니다.")

pay=50000 
kakao=kakaoPay()
kakao.authenticate()
kakao.process_payment(pay)
kakao.payment_info(pay)'''



